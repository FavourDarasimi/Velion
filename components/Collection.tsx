...
function CarCard({ car }: { car: Car }) {
  ...
  return (
    <article
      data-reveal
      className="group flex flex-col overflow-hidden rounded-2xl border border-[#E2E8F0] dark:border-white/10 bg-white dark:border-white/10 dark:bg-slate-900 transition-shadow duration-300 hover:shadow-[0_24px_50px_-30px_rgba(15,23,42,0.4)]"
    >
      <div className="relative aspect-[16/10] overflow-hidden">
        ...
        {car.badge && (
          <span className="absolute top-3 left-3 rounded-full bg-white px-3 py-1 font-mono text-[10px] font-bold tracking-[0.2em] text-[#0F172A] dark:text-slate-100 uppercase shadow-sm">
            {car.badge}
          </span>
        )}
      ...
      <div className="flex flex-1 flex-col p-5 sm:p-6">
        <p className="font-mono text-[11px] tracking-[0.2em] text-[#64748B] dark:text-slate-400 uppercase">
          {car.tagline}
        </p>
        <h3 className="font-display mt-2 text-2xl font-semibold tracking-tight text-[#0F172A] dark:text-slate-100">
          {car.name}
        </h3>
        <div className="mt-4 flex items-baseline justify-between border-t border-[#E2E8F0] dark:border-white/10 pt-4">
          <p className="font-display text-xl font-semibold text-[#0F172A] dark:text-slate-100">
            {car.price}
          </p>
          <p className="font-mono text-[11px] tracking-[0.12em] text-[#64748B] dark:text-slate-400">
            {car.monthly}
          </p>
        </div>
        <a
          href={DIALOG_HASH}
          className="mt-4 inline-flex items-center justify-center rounded-full bg-[#0F172A] dark:bg-white dark:text-slate-950 dark:hover:bg-slate-200 px-5 py-2.5 text-sm font-semibold text-white transition-colors duration-200 hover:bg-[#1E293B] focus-visible:ring-2 focus-visible:ring-[#0F172A] focus-visible:ring-offset-2 focus-visible:outline-none"
        >
          View car
        </a>
      </div>
    ...
    >
      <div className="mx-auto w-full max-w-[1500px] px-4 sm:px-6 lg:px-10">
        ...
        >
          ...
          <p className="font-mono text-[11px] tracking-[0.24em] text-[#94A3B8] dark:text-slate-500 uppercase">
            ...
            return (
              <button
                ...
                className={`flex cursor-pointer items-center gap-2 rounded-full border px-4 py-2 text-sm transition-colors duration-200 focus-visible:ring-2 focus-visible:ring-[#0F172A] focus-visible:outline-none ${
                  isActive
                    ? "border-[#0F172A] bg-[#0F172A] dark:bg-white dark:text-slate-950 font-semibold text-white"
                    : "border-[#E2E8F0] dark:border-white/10 bg-white dark:border-white/10 dark:bg-white/5 dark:text-slate-200 font-medium text-[#475569] dark:text-slate-300 hover:border-[#0F172A] hover:text-[#0F172A] dark:text-slate-100"
                }`}
              >
                {f}
                <span
                  className={`font-mono text-[11px] ${isActive ? "text-white/70" : "text-[#94A3B8] dark:text-slate-500"}`}
                >
                  {countFor(f)}
                </span>
              </button>
        ...
        {visible.length > 0 ? (
          ...
          <div
            data-reveal
            className="mt-8 rounded-2xl border border-dashed border-[#CBD5E1] dark:border-white/15 dark:bg-white/5 bg-white/60 px-6 py-16 text-center"
          >
            <p className="font-display text-2xl font-semibold tracking-tight text-[#0F172A] dark:text-slate-100">
            ...
            <button
              ...
              className="mt-6 cursor-pointer rounded-full bg-[#0F172A] dark:bg-white dark:text-slate-950 dark:hover:bg-slate-200 px-6 py-2.5 text-sm font-semibold text-white transition-colors hover:bg-[#1E293B] focus-visible:ring-2 focus-visible:ring-[#0F172A] focus-visible:ring-offset-2 focus-visible:outline-none"
            >
              Browse everything
            </button>
          </div>
